...
function* api_v1_hghfhgjh_listWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hghfhgjh_list, action)
    yield put(actions.api_v1_hghfhgjh_listSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hghfhgjh_listFailed(err, action))
  }
}
function* api_v1_hghfhgjh_listWatcher() {
  yield takeEvery(types.API_V1_HGHFHGJH_LIST, api_v1_hghfhgjh_listWorker)
}
function* api_v1_hghfhgjh_createWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hghfhgjh_create, action)
    yield put(actions.api_v1_hghfhgjh_createSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hghfhgjh_createFailed(err, action))
  }
}
function* api_v1_hghfhgjh_createWatcher() {
  yield takeEvery(types.API_V1_HGHFHGJH_CREATE, api_v1_hghfhgjh_createWorker)
}
function* api_v1_hghfhgjh_readWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hghfhgjh_read, action)
    yield put(actions.api_v1_hghfhgjh_readSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hghfhgjh_readFailed(err, action))
  }
}
function* api_v1_hghfhgjh_readWatcher() {
  yield takeEvery(types.API_V1_HGHFHGJH_READ, api_v1_hghfhgjh_readWorker)
}
function* api_v1_hghfhgjh_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hghfhgjh_update, action)
    yield put(actions.api_v1_hghfhgjh_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hghfhgjh_updateFailed(err, action))
  }
}
function* api_v1_hghfhgjh_updateWatcher() {
  yield takeEvery(types.API_V1_HGHFHGJH_UPDATE, api_v1_hghfhgjh_updateWorker)
}
function* api_v1_hghfhgjh_partial_updateWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hghfhgjh_partial_update, action)
    yield put(actions.api_v1_hghfhgjh_partial_updateSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hghfhgjh_partial_updateFailed(err, action))
  }
}
function* api_v1_hghfhgjh_partial_updateWatcher() {
  yield takeEvery(
    types.API_V1_HGHFHGJH_PARTIAL_UPDATE,
    api_v1_hghfhgjh_partial_updateWorker
  )
}
function* api_v1_hghfhgjh_deleteWorker(action) {
  try {
    const result = yield call(apiService.api_v1_hghfhgjh_delete, action)
    yield put(actions.api_v1_hghfhgjh_deleteSucceeded(result, action))
  } catch (err) {
    yield put(actions.api_v1_hghfhgjh_deleteFailed(err, action))
  }
}
function* api_v1_hghfhgjh_deleteWatcher() {
  yield takeEvery(types.API_V1_HGHFHGJH_DELETE, api_v1_hghfhgjh_deleteWorker)
}
...
export default function* rootSaga() {
  const sagas = [
    ...
    api_v1_hghfhgjh_listWatcher,
    api_v1_hghfhgjh_createWatcher,
    api_v1_hghfhgjh_readWatcher,
    api_v1_hghfhgjh_updateWatcher,
    api_v1_hghfhgjh_partial_updateWatcher,
    api_v1_hghfhgjh_deleteWatcher,
